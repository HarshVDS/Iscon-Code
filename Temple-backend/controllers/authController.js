...
export const login = async (req, res) => {
  try {
    ...
    const token = generateToken(user);
    res.json({ token, user: { id: user._id, fullName: user.fullName, email: user.email, mobile: user.mobile } });
  } catch (err) {
    ...
    html: `
      <div style="font-family: Arial, sans-serif; padding: 20px; border: 1px solid #eaeaea; border-radius: 8px; background-color: #f9f9f9;">
        ...
        <p style="font-size: 14px; color: #888;">
          ...
          For support, contact us at <a href="mailto:${process.env.MAIL_USER}">${process.env.MAIL_USER}</a>.
        </p>
      </div>
    `,
  });
...
export const resetPassword = async (req, res) => {
  ...
  // Update user fields directly to avoid pre-save hook
